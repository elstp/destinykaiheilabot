...
public class HttpClientUtils {
    ...
    private final static int CONNECT_TIMEOUT = 40000;// 连接超时毫秒
    ...
    private final static String RESP_CONTENT_JSON = "{\"code\":1,\"msg\":\"请求失败!请重试!\"}";
     ...
     * 集
     */
    public static String sendGetRequest(String reqURL, String param,Map<String,String> header) {
        if (null != param) {
            reqURL += "?" + param;
        }
        String respContent = RESP_CONTENT_JSON; // 响应内容
    ...
    private static String errMsg(String msg){
        jsonObject.clear();
     ...
     */
    public static String sendPostRequest(String reqURL, String param, String type,Map<String,String> header) {
        String result = RESP_CONTENT_JSON;
        // 设置请求和传输超时时间
        ...
        try {
            ...
            logger.error("请求通信[" + reqURL + "]时连接超时,堆栈轨迹如下", cte);
            result = errMsg(cte.getMessage());
        } catch (SocketTimeoutException ste) {
            logger.error("请求通信[" + reqURL + "]时读取超时,堆栈轨迹如下", ste);
            result = errMsg(ste.getMessage());
        } catch (ClientProtocolException cpe) {
            logger.error("请求通信[" + reqURL + "]时协议异常,堆栈轨迹如下", cpe);
            result = errMsg(cpe.getMessage());
        } catch (ParseException pe) {
            logger.error("请求通信[" + reqURL + "]时解析异常,堆栈轨迹如下", pe);
            result = errMsg(pe.getMessage());
        } catch (IOException ioe) {
            logger.error("请求通信[" + reqURL + "]时网络异常,堆栈轨迹如下", ioe);
            result = errMsg(ioe.getMessage());
        } catch (Exception e) {
            logger.error("请求通信[" + reqURL + "]时偶遇异常,堆栈轨迹如下", e);
     ...
     * @return
     */
    public static Map<String,String> doHuaSHengCookiePost(String url,Map<String, String> map, String charset) {
        ...
        try {
            ...
            for (int i = 0; i < cookies.size(); i++) {
                ...
                if (cookies.get(i).getName().equals("huasyuncomsaveNull")) {
        ...
        data.put("PHPSESSID",JSESSIONID);
        data.put("huasyuncomsaveNull",cookie_user);
        return data;
    }

    public static String sendHuaSHengPostRequest(String reqURL, Map<String,String>  param, String type,Map<String,String> header,String domain,Map<String,String> cookieList) {
        String result = RESP_CONTENT_JSON;
        // 设置请求和传输超时时间
        HttpPost httpPost = new HttpPost(reqURL);
        // 这就有可能会导致服务端接收不到POST过去的参数,比如运行在Tomcat6.0.36中的Servlet,所以我们手工指定CONTENT_TYPE头消息
        ...
        try {
            ...
            for(Map.Entry<String, String> entry : cookieList.entrySet()){
                ...
                BasicClientCookie cookie = new BasicClientCookie(mapKey, mapValue);
                cookie.setVersion(0);
